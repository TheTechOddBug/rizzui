...
#### Variant Utility (`createVariant`)

The `createVariant` utility provides a type-safe way to create component variants with Tailwind CSS classes. It's the same utility used internally by RizzUI components.

**Key Features:**

- Type-safe variant definitions with full TypeScript inference
- Supports boolean, string, and number variants
- Compound variants for conditional styling
- Slots support for multi-part components
- Zero runtime dependencies (except `cn`)
